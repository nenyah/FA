...
namespace PartAssignment.com.amtec.SelectGW
{
    public partial class SelectGW
    {
        ...
        public ExecutionResult CheckWorkOrderContainSN(string order)
        {
            exeRes.Status = true;
            DataSet ds = new DataSet();
            string sql = @"SELECT * FROM GLO.CHARGE_SNR GL JOIN BDE.CHARGE CH ON (gl.CHARGE_NR = ch.CHARGE_NR) JOIN GLO.ADIS AD ON (ch.OBJECT_ID = ad.OBJECT_ID) where ch.charge_ext ='" + order + "'";
            OracleDataAdapter dataAdapter = new OracleDataAdapter();
            OracleCommand command = new OracleCommand();
            command.Connection = oraconn;
            if (!(command.Connection.State == ConnectionState.Open))
  ...
  from bde.v_auft v
 where V.bea_status = '" + status + "' ORDER BY V.charge_created DESC";
            OracleDataAdapter dataAdapter = new OracleDataAdapter();
            OracleCommand command = new OracleCommand();
            command.Connection = oraconn;
            if (!(command.Connection.State == ConnectionState.Open))
        ...
        public ExecutionResult GetMaGrpIDM(string ma_grp_nr)
        {
            exeRes.Status = true;
            DataSet ds = new DataSet();
            string sql = @"select ma_grp_id from bde.ma_grp mg where mg.MA_GRP_TYP = 'M' and ma_grp_nr = '" + ma_grp_nr + "'";
            OracleDataAdapter dataAdapter = new OracleDataAdapter();
            OracleCommand command = new OracleCommand();
            command.Connection = oraconn;
            if (!(command.Connection.State == ConnectionState.Open))
        ...
        public ExecutionResult GetMaGrpIDT(string ma_grp_nr)
        {
            exeRes.Status = true;
            DataSet ds = new DataSet();
            string sql = @"select ma_grp_id from bde.ma_grp mg where mg.MA_GRP_TYP = 'T' and ma_grp_nr = '" + ma_grp_nr + "'";
            OracleDataAdapter dataAdapter = new OracleDataAdapter();
            OracleCommand command = new OracleCommand();
            command.Connection = oraconn;
            if (!(command.Connection.State == ConnectionState.Open))
        ...
        public ExecutionResult GetStorageGroup(string dbType,string cell_id)
        {
            exeRes.Status = true;
            DataSet ds = new DataSet();
            string sql = @"SELECT LAGER_GRP_NR FROM ML.LAGER_GRP WHERE STORAGE_CELL_ID IN (SELECT ID FROM ml.storage_cell WHERE CELL_NUMBER = '"+ cell_id +"')";
            if (dbType == "SQL Server")
            {
                SqlDataAdapter dataAdapter = new SqlDataAdapter();
                SqlCommand command = new SqlCommand();
        ...
        public ExecutionResult DeletePartLocation(string dbType, string part_no, string location_lager_id, string part_object_id)
        {
            exeRes.Status = true;
            DataSet ds = new DataSet();
            if (dbType == "SQL Server")
        ...
        public string GetStorageCellID(string dbType,string cell_no)
        {
            ...
            if (dbType == "SQL Server")
            {
                string sql = @"select ID from ml.STORAGE_CELL where CELL_NUMBER = '"+ cell_no +"' AND PLANT_ID = '3000000'";
                SqlDataAdapter dataAdapter = new SqlDataAdapter();
                SqlCommand command = new SqlCommand();
                command.Connection = conn;
                if (!(command.Connection.State == ConnectionState.Open))
        ...
        public string GetLagerGrpID(string dbType,string cell_id,string group_no)
        {
            ...
            if (dbType == "SQL Server")
            {
                string sql = @"SELECT LAGER_GRP_ID FROM ML.LAGER_GRP WHERE LAGER_GRP_NR = '"+group_no+"' AND STORAGE_CELL_ID = '"+cell_id+"' and WERK_ID = '3000000'";
                SqlDataAdapter dataAdapter = new SqlDataAdapter();
                SqlCommand command = new SqlCommand();
                command.Connection = conn;
                if (!(command.Connection.State == ConnectionState.Open))
        ...
        public string GetBurn(string dbType, string Version, string hsn)
        {
            ...
            if (dbType == "MySQL")
            {
                string sql = @"select node_data from ict where  serialNum = '" + hsn + "' order by date_time limit 1";
                MySqlDataAdapter dataAdapter = new MySqlDataAdapter();
                MySqlCommand command = new MySqlCommand();
                command.Connection = con;
                if (!(command.Connection.State == ConnectionState.Open))
        ...
        public string GetHigh(string dbType, string Version, string hsn)
        {
            ...
            if (dbType == "MySQL")
            {
                string sql = @"select node_data from hightemtest where serialNum = '" + hsn + "' order by date_time limit 1";
                MySqlDataAdapter dataAdapter = new MySqlDataAdapter();
                MySqlCommand command = new MySqlCommand();
                command.Connection = con;
                if (!(command.Connection.State == ConnectionState.Open))
        ...
        public string GetEol(string dbType, string Version, string hsn)
        {
            ...
            if (dbType == "MySQL")
            {
                string sql = @"select node_data from normaltest where serialNum = '" + hsn + "' order by date_time limit 1";
                MySqlDataAdapter dataAdapter = new MySqlDataAdapter();
                MySqlCommand command = new MySqlCommand();
                command.Connection = con;
                if (!(command.Connection.State == ConnectionState.Open))
